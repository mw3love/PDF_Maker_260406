...
| Phase | 제목 | 상태 |
|-------|------|------|
| 1 | 프로젝트 기반 + converter.py | [x] 완료 |
| 2 | install.py (레지스트리 등록/해제) | [x] 완료 |
...
- [x] **기본 동작 검증**
  ...
  - **비고**: converter.py 버그 수정 — `show_pdf_page` → `insert_image` (이미지 문서는 PDF가 아니므로), merge_files에 `is_pdf` 분기 추가.
...
- [x] **세션 수집기 검증**
  ...
  - **비고**: lock 파일 선점 로직 코드 검토 완료. 실제 다중 실행은 GUI 환경에서 수동 확인 필요.
...
- [x] **엣지 케이스 검증**
  ...
  - **비고**: ① 한글 경로 PASS ② 충돌 _1/_2 PASS ③④⑤ GUI/코드 검토 완료.

- [x] **레지스트리 등록 후 실제 우클릭 검증**
  ...
  - **비고**: merge 등록 범위를 `*` → `SystemFileAssociations\.{jpg,jpeg,png,bmp,pdf}`로 변경 (실행파일 등 불필요한 파일 제외).
