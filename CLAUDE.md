...
- `show_result_popup(parent, title, message, outputs, auto_open_folder=False, show_folder_button=True, show_popup=True)`: 변환/병합 **완료 처리**(기존 `messagebox.showinfo` 대체). **PDF는 항상 자동으로 연다**(`os.startfile`, 버튼 불필요 — 2026-08-03: 완료 즉시 열리도록 변경, 이전엔 `[PDF 열기]` 버튼 클릭이 필요했음). `auto_open_folder=True`면 폴더도 자동으로 연다. `show_folder_button`이 True일 때만 `[폴더 열기]` 버튼(`explorer /select,<첫 파일>`) 노출 — **사전 설정 창(체크박스)이 없는 흐름에서만 True**(우클릭 단일파일 병합·우클릭 일괄변환). `[닫기]` 버튼은 제거(우상단 X로 대체). `outputs` 중 실제 존재하는 것만 열기 대상. `Enter`/`ESC` 둘 다 닫기. `wait_visibility()`→`grab_set()`→`wait_window()`로 모달(호출측이 이후 `root.destroy()` 해도 클릭 시간 보장).
  - **`show_popup=False`면 팝업 창 자체를 안 띄운다** (2026-08-03) — PDF 자동 열기가 이미 확인 수단이므로 **정상 성공 시엔 팝업 생략**, **에러/부분실패(errors 있음)일 때만 `show_popup=True`**로 팝업 표시(실패 파일 목록은 팝업 없이는 알 수 없음). 호출측(`_run_merge_popup`·`main.py`·`HelperWindow`)이 각자 `bool(errors)`로 판단해 전달. 실측(2026-08-03): 우클릭 다중파일 병합은 성공 시 팝업이 없어지면서 `MergeWindow._start_merge` → `on_done_extra`(`status=="done"`이면 `self._cancel()`)가 곧바로 실행돼, **창뿐 아니라 `pdf_maker.exe` 프로세스 자체가 클릭 없이 자동 종료**됨을 실제 exe로 확인(SendKeys로 병합 시작 → PDF 뷰어만 뜨고 `pdf_maker.exe` 프로세스 0개로 종료).
- **"완료 후 폴더 열기" 체크박스** (2026-08-03): `MergeWindow`(우클릭 다중파일 병합)와 `HelperWindow`(더블클릭, 병합·변환 공통)에 기본 off 체크박스 추가. 값은 `_run_merge_popup(open_folder_default=...)` / `show_result_popup(auto_open_folder=...)`로 전달되고, 이 두 창을 거치는 흐름은 `show_folder_button=False`(체크박스가 그 결정을 대신하므로 완료 팝업엔 폴더 열기 버튼을 두지 않음). 사전 설정 창이 없는 흐름(우클릭 단일파일 병합·우클릭 일괄변환)은 실패 시에만 뜨는 완료 팝업의 `[폴더 열기]` 버튼에 의존(성공 시엔 팝업 자체가 없어 그 버튼도 없음).
- **`HelperWindow`(도우미 GUI)는 의도적으로 작업 완료 후에도 계속 열려있다** — 여러 배치를 연속 처리하는 용도라 `MergeWindow`처럼 자동 종료하지 않는다(2026-08-03 확인, 기존 설계 유지 결정).
