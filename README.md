...
Windows 탐색기 우클릭 메뉴에서 이미지·HWP→PDF 변환 및 PDF 병합을 수행하는 유틸리티.
...
![이미지 파일 우클릭 메뉴](docs/images/image_context_menu.png)

### HWP → PDF 변환
- 탐색기에서 한글(`.hwp`, `.hwpx`) 파일 선택 → 우클릭 → **"HWP → PDF 변환"**
- 한컴오피스(한글) 설치 필요, 다중 선택 시 한컴 COM 세션 1개로 일괄 변환
- 메뉴 등록 시 보안 모듈이 함께 등록되어 변환 중 "모두 허용" 팝업 없음
- 출력: 원본 파일과 같은 폴더, 파일명 충돌 시 `_1`, `_2` 자동 suffix

![HWP 파일 우클릭 메뉴](docs/images/hwp_context_menu.png)

### PDF 병합
- 파일 선택(이미지·PDF·HWP 혼합 가능) → 우클릭 → **"PDF로 병합"**
- 병합 GUI에서 순서 조정(▲▼), 파일 추가/제거 후 실행
- 파일 1개 선택 시 GUI 없이 즉시 처리
- 출력: `merged.pdf` (충돌 시 `merged_1.pdf`, `merged_2.pdf`...)

![PDF 병합 GUI](docs/images/merge_gui.png)

![PDF 병합 결과](docs/images/merge_result.png)
...
| 날짜 | 내용 |
|------|------|
| 2026-08-25 | 스크린샷 4종 추가, 누락돼 있던 "HWP → PDF 변환" 섹션 신규 작성 |
| 2026-08-03 | 성공 시 완료 팝업 생략(에러 시엔 유지) — 우클릭 병합은 성공하면 창·프로그램까지 자동 종료 |
| 2026-08-03 | 변환/병합 완료 시 PDF 자동 열기, 완료 후 폴더 열기 체크박스(기본 off) 추가 |
